...
public class ExeCommand {
    ...
    public ExeCommand(ExeCommand cmdOld) {
		this(false);
		//CMN.Log("PlayService","reConstruct @@pre0完成!!!");
    	finallyDo = cmdOld.finallyDo;
    	cmdOld.stop();
		//CMN.Log("PlayService","reConstruct @@pre1完成!!!");
    	cmdl = cmdOld.cmdl;
		CMN.Log("PlayService","reConstruct @@pre2完成!!!");
	}
    ...
    public int run(String...commands) {
    	if(stopped)  init();
		CMN.Log("PlayService", "!!!running command:",commands);
		stopped = false;
    	bRunning = true;
				...
				try {
					while ((line=errorResult.readLine()) != null) {
						if(stopped) break;
							...
							CMN.Log("fatal PlayService","CountorThread : "+line);
							line += "\n";
							if(!stopped && cmdl!=null) cmdl.OnOutput(line);
							//writeLock.lock(); result.append(line); writeLock.unlock();
					}
				} catch (Exception e) {
					CMN.Log("PlayService", "read ErrorStream exception:" + e.toString());
					line=null;
					e.printStackTrace();
				}
				CMN.Log("read ErrorStream 结束 ");
			});
            CountorThread.start();
