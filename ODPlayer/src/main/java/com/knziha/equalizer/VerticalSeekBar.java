...
import com.knziha.ODPlayer.BSTNode;
import com.knziha.ODPlayer.BSTree;
...
public class VerticalSeekBar extends SeekBar{
    ...
    private void inOrderDraw(BSTNode<myCpr> node, Canvas canvas) {
        if(node != null && timeLength != 0) {
            inOrderDraw(node.left, canvas);
            ...
            if(node.getKey().value.equals("")){
                mPaint.setColor(0xFFFF00FF);
                canvas.drawLine(xLeft +(float)node.getKey().key/(float)timeLength*(-xLeft + xRight), 0, xLeft +(float)node.getKey().key/(float)timeLength*(-xLeft + xRight), getMeasuredWidth(), mPaint);
            }else {
                mPaint.setColor(Color.CYAN);
                drawText(canvas, node.getKey().value,null, xLeft + (float) node.getKey().key / (float) timeLength * (-xLeft + xRight), 10.f, mPaint, -90.f);
            }
            inOrderDraw(node.right, canvas);
        }
    }
