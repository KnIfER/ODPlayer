    ...
    private void inflateSelectionPool() {
        if(selStart!=selEnd){
            ...
            if(textCover2!=null){
                ...
                int cc=0;
                boolean isDrawSimple = VICMainAppOptions.isDrawSimpleSelection();
...
//                                getLayout().getOffsetForHorizontal(lineEnd,0),mSEnd)));
    ...
    public void instantiate(SelectableTextViewCover mSelectableTextViewCover, SelectableTextViewBackGround mSelectableTextViewBackGround, ScrollViewHolder scroller, FrameCover cover3) {
        ...
        textCover.magW = (int) (83*getResources().getDisplayMetrics().density*(VICMainAppOptions.isLarge?2:1));
        sv.tv2guard = this;
    }
...
//                    int line = getLayout().getLineForVertical((int) selStartPosY);
    ...
    public void setTheme(int bgColor, int textColor, int heightLightColor) {
        BackGroundColor = bgColor;
        setTextColor(Color.BLACK);
        if(textCover2!=null)textCover2.highLightBg=heightLightColor;
        handleLeft.setColorFilter(heightLightColor, PorterDuff.Mode.SRC_IN);
        handleRight.setColorFilter(heightLightColor, PorterDuff.Mode.SRC_IN);
    }
