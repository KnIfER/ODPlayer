...
public class ColorPickerDialog extends DialogFragment implements
	...
	@Override
	public void dismiss() {
		if(oldColor!=colors.get(selectedPosition))
			putColor(selectedPosition, colors.get(selectedPosition));
		if(isDirty){
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		CMNF.Log("ColorPickerDialog - onCreateDialog", dialogId);
		...
		createColorShades(colors.get(selectedPosition));

		adapter = new ColorPaletteAdapter(new OnColorSelectedListener() {
			@Override public void onColorSelected(int Pos) {
		...
		rootView.findViewById(R.id.confirm).setOnClickListener(v -> {
			colorPickerDialogListener.onColorSelected(dialogId, colors.get(selectedPosition));
			dismiss();
		});
		//自定义颜色
		rootView.findViewById(R.id.color).setOnClickListener(v -> {
			...
			if(isPickerAttached) {
				...
				if(gridView.isExpanded()) {//collapse
					gridView.setExpanded(false);//TODO增加高度判断
					gridView.post(() -> {
						if(false){
							gridView.smoothScrollToPositionFromTop(collapsePosition, collapseOffset);
						}else {
							if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
								gridView.setSelectionFromTop(collapsePosition, collapseOffset);
							} else {
				...
				if(colorViewPicker.getColor()!=colors.get(selectedPosition))
					colorViewPicker.setColor(colors.get(selectedPosition));
				...
				lp.height = (int) (Math.min(dm.widthPixels, dm.heightPixels) - 32 * dm.density);
				colorViewPicker.setLayoutParams(lp);//正方形才舒心嘛！
				if(dm.heightPixels<views_holder0.getHeight()+dm.widthPixels){//expand
	...
	@Override public void onSaveInstanceState(Bundle outState) {
		...
		if(initialPosition==-1 && colors.size()>0)
			getArguments().putInt(ARG_Initial_Color, colors.get(0));
	...
	public void onViewStateRestored(@Nullable Bundle savedInstanceState) {
		CMNF.Log("ColorPickerDialog - onViewStateRestored", dialogId);
	...
	View createPickerView() {
		...
		oldColorPanel.setColor(colors.get(selectedPosition));
		if(saberColor!=-1) {
			colorViewPicker.setColor(saberColor, true);
			newColorPanel.setColor(saberColor);
			setHex(saberColor);
		}else {
			colorViewPicker.setColor(colors.get(selectedPosition), true);
			newColorPanel.setColor(colors.get(selectedPosition));
			setHex(colors.get(selectedPosition));
		}
	...
	private void loadPresets() {
		...
		oldColor=colors.get(selectedPosition);
	}
		...
		for (final int colorShade : colorShades) {
			...
			colorPanelView.setOnClickListener(v -> {//shade 子项
				...
				if (v.getTag() instanceof Boolean && (Boolean) v.getTag()) {
					colorPickerDialogListener.onColorSelected(dialogId, colorPanelView.getColor());//�˴������������顪��what?
	...
	public void onActivityCreated(Bundle s) {
		...
		//int colors
		//Settings.getInstance(getActivity()).getColor()
			...
			private void setOnClickListener(View itemView) {//核心点击事件
				itemView.setOnClickListener(v -> {
					...
					if(saberColor!=-1) {
						saberColor=-1;
						if(isPickerAttached) {
							colorViewPicker.setColor(colors.get(selectedPosition), false);
							oldColorPanel.setColor(colors.get(selectedPosition));
							newColorPanel.setColor(colors.get(selectedPosition));
						}
					}
						...
						if(colorPickerDialogListener!=null)
							colorPickerDialogListener.onColorSelected(dialogId, colors.get(Pos));
						dismiss();
						return;
					}

					if(oldColor!=colors.get(selectedPosition))//离巢之鸟 新存乡颜
						putColor(selectedPosition, colors.get(selectedPosition));

					//listener.onColorSelected(Pos);

					Integer newColor = colors.get(Pos);
					if(isPickerAttached) {
						colorViewPicker.setColor(newColor, false);
